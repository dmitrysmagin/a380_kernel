...
//		 .fg0 = {32, 0, 0, 400, 240}, /* bpp, x, y, w, h */
		...
		.panel = {
			...
			.slcd_cfg = SLCD_CFG_DWIDTH_16BIT | SLCD_CFG_CWIDTH_16BIT | SLCD_CFG_CS_ACTIVE_LOW | SLCD_CFG_RS_CMD_LOW | SLCD_CFG_CLK_ACTIVE_FALLING | SLCD_CFG_TYPE_PARALLEL,
			//.ctrl = LCD_CTRL_OFUM | LCD_CTRL_BST_16 | LCD_CTRL_BPP_18_24,	/* 16words burst, enable out FIFO underrun irq */
			//.ctrl = LCD_CTRL_OFUM | LCD_CTRL_BST_16 | LCD_CTRL_BPP_18_24,	/* 16words burst, enable out FIFO underrun irq */
		...
		.osd = {
			...
			//		 LCD_OSDC_ALPHAMD,	/* enable Foreground0 */
			.osd_ctrl = 0,		/* disable ipu,  */
			.rgb_ctrl = 0,
			.bgcolor = 0x000000, /* set background color Black */
			.colorkey0 = 0, /* disable colorkey */
...
#if defined (CONFIG_FB_JZ4750_SLCD) //need to check again 
	/* Foreground 1 */
...
//for backlight, need to re-set later
...
static void  __init jz4750fb_drv_cleanup(void)
{
 	struct lcd_cfb_info *cfb = jz4750fb_info;
...
static int __init jz4750fb_drv_init(void)
{
 	struct lcd_cfb_info *cfb;
	int err;
...
static int __init jz4750fb_drv_register(void)
{
 	struct lcd_cfb_info *cfb = jz4750fb_info;
	int err = 0;
...
static void __init jz4750fb_dev_enable(void)
{
...
static int __init jz4750fb_dev_init(void)
{
#if defined (CONFIG_FB_JZ4750_SLCD)
	__gpio_as_lcd_16bit();

...
#include "jz4750_slcd.h"
#include "jz4750_tve.h"

#ifdef CONFIG_JZ4750_SLCD_400X240_8352B
#include "400x240_8352B.h"
#endif

MODULE_DESCRIPTION("Jz4750 SLCD Controller driver");
MODULE_AUTHOR("kaka, <android@gmail.com>");
MODULE_LICENSE("GPL");
...
#define JZ_FB_DEBUG 0
	...
	struct jz4750lcd_info jz4750_lcd_panel = {
		...
		.panel = {
			.cfg = LCD_CFG_LCDPIN_SLCD | LCD_CFG_NEWDES | LCD_CFG_MODE_SLCD, /* TFT Smart LCD panel */
			...
			.ctrl =  LCD_CTRL_BST_16 |LCD_CTRL_BPP_18_24,	/* 16words burst, enable out FIFO underrun irq */
...
static void jz4750fb_deep_reset_mode( struct jz4750lcd_info * lcd_info );
...
//kaka
static void ctrl_reenable(void)
{
	REG_LCD_STATE = 0; /* clear lcdc status */
	__lcd_slcd_special_reset();
	//__lcd_slcd_special_on();
	__lcd_clr_dis();
	__lcd_set_ena(); /* enable lcdc */
...
static int jz4750fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case FBIO_DEEP_SET_MODE:
			printk("fbio deep set mode\n");

			if (copy_from_user(jz4750_lcd_info, argp, sizeof(struct jz4750lcd_info)))
				return -EFAULT;

			jz4750fb_deep_reset_mode(jz4750_lcd_info);

			break;

#ifdef CONFIG_FB_JZ4750_TVE
		case FBIO_MODE_SWITCH:
			printk("FBIO_MODE_SWITCH\n");
			switch (arg) {
...
#if defined(CONFIG_FB_JZ4750_SLCD_USE_2LAYER_FRAMEBUFFER)
...
static void jz4750fb_reset_panel_mode( struct jz4750lcd_info * lcd_info )
{
	struct jz4750lcd_panel_t *panel = &lcd_info->panel;
#ifdef CONFIG_JZ4750D_VGA_DISPLAY
	REG_TVE_CTRL |= TVE_CTRL_DAPD;
...
static void jz4750fb_deep_reset_mode( struct jz4750lcd_info * lcd_info )
{
	...
	printk("deep reset mode \n");

	__lcd_clr_ena();	/* Quick Disable */
	lcd_info->osd.fg_change = FG_CHANGE_ALL; /* change FG0, FG1 size, postion??? */
	jz4750fb_descriptor_init(lcd_info);

	jz4750fb_reset_panel_mode(lcd_info);
	//jz4750fb_set_panel_mode(lcd_info);
	jz4750fb_set_mode(lcd_info);
	jz4750fb_change_clock(lcd_info);
	__lcd_set_ena();	/* enable lcdc */
	printk("deep reset mode end \n");
}
...
{
	backlight_level = 80; //simple_strtoul(buffer, 0, 10);
	return count;
}

static int lcd_flush_read_proc(char *page, char **start, off_t off, 
		int count, int *eof, void *data)
{
	int len =0;
	if (1)
...
static int lcd_flush_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
...
static int __devinit jz4750_fb_probe(struct platform_device *dev)
{
	...
	cfb = jz4750fb_alloc_fb_info(); //alloc memory first
	if (!cfb)
		goto failed;
	...
	if (res) {
		//res->owner = THIS_MODULE;
		res->read_proc = lcd_flush_read_proc;
		res->write_proc = lcd_flush_write_proc;
	}

...
#undef DEBUG_L009
#ifdef DEBUG_L009
#define dprintk(x,s...)  {printk("%s:%d ",__FILE__,__LINE__);printk(x,##s);}
#else
#define dprintk(x,s...) 
#endif
...
int audio_get_endpoint_freesize(audio_pipe *endpoint, audio_buf_info *info)
{
	...
	info->bytes = count * endpoint->fragsize;
        //printk("info->fragments is %d info->fragstotal is %d info->fragsize is %d info->bytes is %d\n",info->fragments,info->fragstotal,info->fragsize,info->bytes);
	return info->bytes;
}
...
{
  dprintk("%s %d pagesize is %d count is %d\n",__FILE__,__LINE__,pagesize,count);
...
{
        dprintk("pagesize is %d count is %d \n",pagesize, count);
...
static int jz_audio_release(struct inode *inode, struct file *file)
{
	...
	ENTER();
        ...
        //newfragstotal = 4;
        newfragsize = JZCODEC_RW_BUFFER_SIZE * PAGE_SIZE;
        newfragstotal= JZCODEC_RW_BUFFER_TOTAL;
        ...
        if (mode & CODEC_RMODE) {
          ...
          audio_resizemem_endpoint(controller->pin_endpoint, newfragsize, newfragstotal);
        }
        if (mode & CODEC_WMODE) {

          audio_resizemem_endpoint(controller->pout_endpoint, newfragsize, newfragstotal);
        }
#endif
...
static int jz_audio_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SNDCTL_DSP_SPEED:
		...
		dprintk("SNDCTL_DSP_SPEED ... set to %d\n", val);
	...
	case SNDCTL_DSP_GETBLKSIZE:
	{
		...
		if (mode & CODEC_WMODE) {
			fragsize = pout_endpoint->fragsize;
                ...
                dprintk("%s %d fragsize is %d\n",__FILE__,__LINE__,fragsize);
		...
		if (rc != -EINVAL) {	
			...
			if (newfragstotal > 32) {
				newfragstotal = 32;
                        ...
                        dprintk("newfragsize is %d newfragstotal is %d mode is 0x%x CODEC_RMODE is 0x%x CODEC_WMODE is 0x%x\n",newfragsize,newfragstotal,mode,CODEC_RMODE,CODEC_WMODE);
			if (mode & CODEC_RMODE) {
...
static int __init init_jz_i2s(struct platform_device *pdev)
{
	...
	fragsize = JZCODEC_RW_BUFFER_SIZE * PAGE_SIZE;
        ...
        dprintk("%s  %s %d fragsize is %d",__FILE__,__FUNCTION__,__LINE__,fragsize);
...
EXPORT_SYMBOL(l009_globle_volume);

...
static void dlv_enable_mic_2(void)
{
	...
	if (first)
	{
		dlv_write_reg(0x1f, 0x0); /* target: 0x0~0x3c */
		dlv_write_reg(0x20, (1 << 7) | (0x2<< 4) | dlv_read_reg(0x20));  /* noise gate[7 6:4] & hold[3:0] */
		dlv_write_reg(0x21, 0x0);			   /* ATK[7:4] and DCY[3:0] */
		dlv_write_reg(0x22, 0x1f);			   /* AGC_MAX */
		dlv_write_reg(0x23, 0x0);			   /* AGC_MIN */
		first = 0;
	}
	__dlv_enable_agc();
#else

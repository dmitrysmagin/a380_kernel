/*
 * linux/arch/mips/jz4750d/board-320e.c
 *
 * JZ4750D 320E board setup routines.
 *
 * Author: <androidkaka@gmail.com>
 *
...
static void a320e_timer_callback(void)
{
...
static void a320e_sd_cpm_start(struct device *dev)
{
	__cpm_start_msc(0);
}
...
static void a320e_tf_cpm_start(struct device *dev)
{
	__cpm_start_msc(1);
}
...
/* SPI devices */
struct spi_board_info a320e_spi0_board_info[]  = {
	[0] = {
		.modalias       = "spidev0",
		.bus_num        = SPI0_BUS,
...
void __init board_spi_init(void){
	jz_add_spi_devices(0,a320e_spi0_board_info,ARRAY_SIZE(a320e_spi0_board_info));
}
...
void __init board_msc_init(void)
{
	...
	jz_add_msc_devices(0, &a320e_sd_data);
#endif

#ifdef CONFIG_JZ_MSC1
	jz_add_msc_devices(1, &a320e_tf_data);
#endif
}

void __init jz_board_setup(void)
{
	printk("JZ4750D 320E board setup\n");

	board_cpm_setup();
	board_gpio_setup();

	jz_timer_callback = a320e_timer_callback;
}

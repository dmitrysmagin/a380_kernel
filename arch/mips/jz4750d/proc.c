...
static int gpio_print (char *page, const char *name, u32 value)
{
	...
	for (i = 0; i < 8; i++) {
		page[len++] = ' ';
		for (j = 0; j < 5; j++, m >>= 1) page[len++] = (value & m) ? '1' : '0';
	}
	page[len++] = '\n';
	page[len] = 0;
	return len;
...
//kaka do test, donot how
static int jz_udc_active;	/* defined in drivers/char/jzchar/jz_udc_hotplug.c */
#endif
...
//kaka add here
#if 1
static int alt_read_proc(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len =0;
	if (1)
...
static int alt_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{

	return count;
}

static int gsensor_read_proc(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len =0;
	if (1)
...
static int gsensor_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{

	return count;
}

static int amp_read_proc(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len =0;
	if (1)
...
static int elan_pad_read_proc(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len =0;
	if (1)
...
static int tvout_read_proc(char *page, char **start, off_t off,
		int count, int *eof, void *data)
{
	int len =0;
	if (1)
...
static int __init jz_proc_init(void)
{
	...
	//kaka 
#if 1 
	res = create_proc_entry("alt", 0644, proc_jz_root);
	if (res){
		res->read_proc = alt_read_proc;
		res->write_proc = alt_write_proc;
		res->data = NULL;
	}

	res = create_proc_entry("gsensor", 0644, proc_jz_root);
	if (res){
		res->read_proc = gsensor_read_proc;
		res->write_proc = gsensor_write_proc;
		res->data = NULL;
	}
	res = create_proc_entry("amp", 0644, proc_jz_root);
	if (res){
		res->read_proc = amp_read_proc;
		res->write_proc = NULL;
		res->data = NULL;
	}
	res = create_proc_entry("elan_pad", 0644, proc_jz_root);
	if (res){
		res->read_proc = elan_pad_read_proc;
		res->write_proc = NULL;
		res->data = NULL;
	}

	res = create_proc_entry("tvout", 0644, proc_jz_root);
	if (res){
		res->read_proc = tvout_read_proc;
		res->write_proc = NULL;
		res->data = NULL;
	}
#endif
